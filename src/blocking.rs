...
/// A strategy for generating blocking clauses that exclude a found model from future checks.
pub trait BlockingStrategy {
    /// Generate a blocking clause that forbids the current model.
    fn generate_blocker(
        &self,
        model: &Model,
        problem: &crate::InferenceProblem,
    ) -> Result<Bool, String>;
}
...
pub struct FixedPointBlocker;

impl BlockingStrategy for FixedPointBlocker {
    fn generate_blocker(
        &self,
        model: &Model,
        problem: &crate::InferenceProblem,
...
/// Block based on uninterpreted function symbols.
///
/// Constructs a blocker that prevents the solver from returning the exact same
/// assignments to all uninterpreted function symbols in the model.
pub struct FunctionSymbolBlocker;

impl BlockingStrategy for FunctionSymbolBlocker {
    fn generate_blocker(
        &self,
        model: &Model,
        problem: &crate::InferenceProblem,
...
impl BlockingStrategy for CombinedBlocker {
    fn generate_blocker(
        ...
        let state_blocker = FixedPointBlocker.generate_blocker(model, problem)?;
